...
def test_cleanup_tool_dry_run_then_apply_preserves_sdk_manager_cache(tmp_path: Path) -> None:
    ...
    stale = data_root / "staging" / ".base-jp6-stale"
    legacy_stale = data_root / "staging" / ".base-jp623-stale"
    stale.mkdir(parents=True)
    legacy_stale.mkdir(parents=True)
    (stale / "partial").write_text("incomplete", encoding="utf-8")
    (legacy_stale / "partial").write_text("incomplete", encoding="utf-8")
...
def test_cleanup_protects_active_staging_and_valid_published_base(
    ...
    data_root = tmp_path / "data"
    active = data_root / "staging" / ".base-jp6-active"
    ...
    inspection = inspect_jp6_base_attempts(data_root)
    ...
    assert not remove_jp6_base_attempts(data_root)
    assert active.is_dir()
    assert valid.is_dir()


def test_cleanup_detects_incomplete_jp6_target_but_ignores_other_family(
    tmp_path: Path,
) -> None:
    data_root = tmp_path / "data"
    ...
    _jp623_lock(
        incomplete / "lock.json",
        "nvidia.jetpack-6.1.jetson-linux-36.4",
    )
    _jp623_lock(other / "lock.json", "nvidia.jetpack-5.1.5.jetson-linux-35.6.1")
    ...
    removed = remove_jp6_base_attempts(data_root)
...
def test_cleanup_does_not_follow_staging_or_target_symlinks(tmp_path: Path) -> None:
    ...
    stale = outside_staging / ".base-jp6-stale"
    ...
    assert not inspect_jp6_base_attempts(data_root).removable
    assert not remove_jp6_base_attempts(data_root)
    assert stale.is_dir()
