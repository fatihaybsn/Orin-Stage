...
def _wheel(
    ...
    *,
    ...
    with zipfile.ZipFile(path, "w") as archive:
        ...
        if orin:
            archive.writestr("orin_stage/__init__.py", '__version__ = "0.2.1"\n')
            archive.writestr(
...
def _locked_wheels(root: Path) -> tuple[dict[str, str], dict[str, str]]:
    ...
    _wheel(root / "runtime", "orin-stage", "0.2.1", orin=True)
...
def test_orin_wheel_requires_catalog_license_and_entry_point(tmp_path: Path) -> None:
    valid = _wheel(tmp_path, "orin-stage", "0.2.1", orin=True)
    wheelhouse.verify_orin_wheel(valid)
    invalid = _wheel(tmp_path, "orin-stage", "0.2.1", build_tag="1")
