...
class FakeSdkManagerClient:
    def __init__(
        ...
        *,
        ...
        self.primary_output = current_output if primary_output is None else primary_output
    ...
    def query_jetson(
        ...
        if archived:
            return self.archived_output
        return self.primary_output if primary_only else self.current_output
...
def test_discovery_records_all_current_visibility_for_jp61() -> None:
    target = _resolver().resolve("jetson-orin@jp6.1")
    client = FakeSdkManagerClient(
        primary_output="""
JetPack 6.2.3
sdkmanager --cli --action install --product Jetson --version 6.2.3 --target JETSON_ORIN_NX_TARGETS --flash
...
""",
    ...
    assert result.target.sdk_manager_display_label == "JetPack 6.1 (rev. 1)"
...
""",
    ...
    assert client.query_calls == [(False, True), (False, False), (True, False)]
