...
def test_jp61_pins_exact_nvidia_meta_closure_for_rolling_repository() -> None:
    target = _target("jetson-orin@jp6.1")

    assert package_seed_names_for_target(target) == JP61_EXACT_META_SEED_NAMES
    seeds = package_seeds_for_target(target)
    assert tuple((seed.name, seed.version, seed.architecture) for seed in seeds[17:]) == (
        JP61_ADDITIONAL_EXACT_SEEDS
    )


def test_jp61_exact_seed_profile_is_used_for_apt_simulation(monkeypatch) -> None:
    target = _target("jetson-orin@jp6.1")
    seeds = package_seeds_for_target(target)
    seen: list[tuple[str, ...]] = []
    ...
    assert seen[0][-1] == "pva-allow-2=2.0.0~rc3"


def test_jp61_has_release_scoped_exact_opencv_removal_policy() -> None:
    policy = package_removal_policy_for_target(_target("jetson-orin@jp6.1"))

    assert policy is not None
    assert policy.version == JP61_REMOVAL_POLICY_VERSION
...
def test_jp621_pins_exact_nvidia_meta_closure_for_rolling_repository() -> None:
    target = _target("jetson-orin@jp6.2.1")

    assert package_seed_names_for_target(target) == JP621_EXACT_META_SEED_NAMES
    seeds = package_seeds_for_target(target)
    assert len(seeds) == 17
    assert all(seed.version == "6.2.1+b38" and seed.architecture == "arm64" for seed in seeds)


def test_jp621_exact_seed_profile_is_used_for_apt_simulation(monkeypatch) -> None:
    target = _target("jetson-orin@jp6.2.1")
    seeds = package_seeds_for_target(target)
    seen: list[tuple[str, ...]] = []
    ...
    assert seen == [tuple(seed.apt_spec for seed in seeds)]
    assert seen[0][0] == "nvidia-jetpack:arm64=6.2.1+b38"


def test_jp621_has_release_scoped_exact_opencv_removal_policy() -> None:
    policy = package_removal_policy_for_target(_target("jetson-orin@jp6.2.1"))

    assert policy is not None
    assert policy.version == JP621_REMOVAL_POLICY_VERSION
    assert policy.jetpack_version == "6.2.1"
    assert policy.l4t_version == "36.4.4"
    assert policy.allowed_removal_set == JP621_ALLOWED_REMOVAL_SET
    assert len(policy.allowed_removal_set) == 19


def test_jp622_pins_exact_nvidia_meta_closure_for_rolling_repository() -> None:
    target = _target("jetson-orin@jp6.2.2")

    assert package_seed_names_for_target(target) == JP622_EXACT_META_SEED_NAMES
    seeds = package_seeds_for_target(target)
    assert len(seeds) == 17
    assert all(seed.version == "6.2.2+b24" and seed.architecture == "arm64" for seed in seeds)


def test_jp622_exact_seed_profile_is_used_for_apt_simulation(monkeypatch) -> None:
    target = _target("jetson-orin@jp6.2.2")
    seeds = package_seeds_for_target(target)
    seen: list[tuple[str, ...]] = []
    ...
    assert seen == [tuple(seed.apt_spec for seed in seeds)]
    assert seen[0][0] == "nvidia-jetpack:arm64=6.2.2+b24"


def test_jp622_has_release_scoped_exact_opencv_removal_policy() -> None:
    policy = package_removal_policy_for_target(_target("jetson-orin@jp6.2.2"))

    assert policy is not None
    assert policy.version == JP622_REMOVAL_POLICY_VERSION
    assert policy.jetpack_version == "6.2.2"
    assert policy.l4t_version == "36.5.0"
    assert policy.allowed_removal_set == JP622_ALLOWED_REMOVAL_SET
    assert len(policy.allowed_removal_set) == 10


@pytest.mark.parametrize("selector", ["jetson-orin@jp6.0", "jetson-orin@jp6.2.3"])
def test_validated_release_package_seed_contract_is_unchanged(selector: str) -> None:
    assert package_seed_names_for_target(_target(selector)) == ("nvidia-jetpack",)
    assert len(package_seeds_for_target(_target(selector))) == 1
