...
def verify_orin_wheel(path: Path) -> None:
    ...
    if metadata.get("Version") != "0.2.1":
        raise WheelhouseError("Orin Stage wheel version is not 0.2.1")
    required = {
...
def build_wheelhouse(
    *,
    ...
    try:
        ...
        print("BUILD orin-stage==0.2.1 role=runtime backend=setuptools.build_meta", flush=True)
        _run(
            (
                str(python),
                "-m",
        ...
        orin_wheel = _find_new_wheel(
            runtime_output, before, "orin-stage", "0.2.1", "runtime"
        )
        ...
        runtime_expected["orin-stage"] = "0.2.1"
